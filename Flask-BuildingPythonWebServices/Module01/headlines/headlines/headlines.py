...
import os
import json
from tvars import OWM_API_KEY, OER_APP_ID
...
def get_weather(query):
    """ Gets the weather from the open weather map api """
    query = quote(query)
    url = WEATHER_URL.format(query, OWM_API_KEY)
...
def get_rate(frm, to):
    """ Calculates currency conversion (poorly, because it just uses the USD instead of direct) """
    all_currencies = urlopen(CURRENCY_URL.format(OER_APP_ID)).read()
